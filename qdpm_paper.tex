...
\begin{figure}[tpb]
    ...
    \caption{Three oriented magnetic strings on the square lattice. From left to right we show: i) a
    nontrivial string winding around the system in one of the two possible directions, ii) an open
    magnetic string with two different charged (colored) visons at the end points, iii) a trivial closed
...
\begin{table}
  \begin{tabular}{| l | c | c | c | c |}
	...
	\hline
    monomer & $0$ & -1 & -2 & $\svrtmon$ \\
	\hline
    dimer & $1$ & 0 & -1 &  $\svrtdmr$ \\
	\hline
    trimer & $2$ & 1 & 0 &  $\svrttrmr$ $\svrttrmrtwo$\\
	\hline
    tetramer & $3$ & 2 & 1 &  $\svrtquat$ \\
	\hline
    pentamer & $4$ & 3 & 2 &  $\svrtpent$ \\
	\hline
  \end{tabular}
...
\begin{table}
  \begin{tabular}{| l | c | }
  	\hline
    $c1$, $\bar{c}1$ & $\pentmvcreate$, $\pentmvanal$ \\
	\hline
  \end{tabular}
  \caption{Defects where $n_v$ dimers touch a vertex and their associated $U(1)$ charges on the A sub-lattice in both the QDM and QFPLM. Defects on the B sub-lattice have opposite charge.}
  \label{tab:defects}
