...
However, at the special points in the $\{(t_\nu,v_\nu)\}$ parameter space, known as the Rokhsar-Kivelson (RK) points,the exact ground state is known when \Eqref{eq:HGQDM} can be written as the sum of projection operators
\begin{equation}
H_{\rm{RK}} = \sum_\nu \hat{h}_\nu \label{eq:HRK},
\end{equation}
...
We define the Hilbert space of the QDPM to be a GQDM where vertices are constrained to be touched by a single dimer or the center of a pentamer, such that $n_v = 1,4$. For defect-free configurations in the QDPM, $Q_v$ only vanishes modulo $3$;  thus we may define a $Z_3$ charge,
\begin{equation}
...
\begin{align}
\MstagSq \equiv  \frac{2}{L^4} \biggl[ &\biggl( \sum_{\ell_h} (-1)^{r_x+r_y} n_d \left(\vec{r}_{\ell_h}\right) \biggr)^2  \notag\\
+&\biggl( \sum_{\ell_v} (-1)^{r_x+r_y} n_d \left(\vec{r}_{\ell_v}\right) \biggr)^2\biggr]\label{eq:Mstag},
\end{align}
\begin{align}
\McolSq \equiv  \frac{2}{L^4} \biggl[  &\biggl( \sum_{\ell_h} (-1)^{r_x} n_d \left(\vec{r}_{\ell_h}\right) \biggr)^2 \notag \\
