...
\begin{figure}[t!]
    \centering
    \includegraphics[width=1.0\columnwidth]{QDPM_ex_config.pdf}
...
\begin{table}
    \begin{tabular}{| l | c | c |}
     ...
     & $c$ & $\bar{c}$ \\
	\hline
    1 & $\pentmvcreate$ &  $\pentmvanal$  \\
	\hline
    2 &$\penmvhrzleft$&$\penmvhrzright$ \\
	\hline
    3 &$\penmvdiagup$& $\penmvdiagdown$\\
	\hline
  \end{tabular}
  \caption{Defects where $n_v$ dimers touch a vertex and their associated $U(1)$ charges on the A sub-lattice in both the QDM and QFPLM. Defects on the B sub-lattice have opposite charge.}
  \label{tab:defects}
...
\begin{figure}[]
    ...
    $32\times 32$ lattice. Different colors correspond to calculations initialized to different winding sectors. Note that $\Phi_X$ is conserved modulo $3$, resulting in a $9$-fold topological degeneracy.}
...
%%%%%%%%%%%%%%%%%%%%%%%
    \begin{figure}
        ...
        \caption{Gray scale of the dimer (left) and $Z_3$ vison (right) correlation functions
            The color blue indicates the dimer (plaquete) from which the correlation is measured. The 
            color red indicate correlations that are off the gray scale used here .}
