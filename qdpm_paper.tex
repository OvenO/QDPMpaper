...
To determine the nature of the ground state at the RK point of the QDPM, we have used Monte Carlo sampling of $\ket{\Psi_{\rm{RK}}}$, using local updates corresponding to the dimer resonances shown in \tabref{tab:pentamer_moves}. Below we present the results of calculations on $L\times L$ square lattices with periodic boundary conditions on systems up to $L=128$. 
...
\begin{figure}[]
    ...
    \caption{The histogram of the $U(1)$ winding number $\Phi_X$ as measured in for the ground state of the RK point of the QDPM for three different lattice sizes. Different colors correspond to calculations initialized to different winding sectors, and the lines are fits to a Gaussian. Note that $\Phi_X$ is conserved modulo $3$, resulting in a $9$-fold topological degeneracy on a torus.}
