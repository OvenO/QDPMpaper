...
Given the gauge symmetry, one may construct a gauge invariant string operator along an oriented path $\Gamma$ path on the dual lattice
\begin{equation}
S_\Gamma \equiv \prod_{\ell \in \Gamma} e^{i \alpha_\ell n_\ell}
\end{equation}
where $\alpha_\ell$ is a phase that is associated with each link on the lattice, as well as the direction the link is crossed in $\Gamma$.

\subsection{Square lattice QDM}

the simplest constraint is that of the hard-core fully-packed QDM where a single dimer must touch each vertex. We write this constraint as $n_v = 1$, 

For example in the QDM $\hat{T}$ is the sum of all possible ``plaquette flip" terms acting on plaquettes with parallel dimers:
\begin{equation}
    \label{}
    \hat{T}_{\rm{QDM}} = \sum_{\bplqt} 
            \left|
                \vrtplqt
            \right\rangle
            \left\langle
