...
Because of the local constraints, all states in the Hilbert space of a GQDM are invariant under local gauge transformations a the vertices, $G_v$~\cite{Moessner2001} of the general form
\begin{equation}
...
\end{equation}
where $\mathrm{sgn}(\ell,\gamma)$ is positive (negative) if $\ell$ is oriented to the right with respect to the orientation of $\gamma$. The string operator is defined as
\begin{equation}
S_\gamma \equiv e^{i \alpha \Phi_\gamma}
\end{equation}
where $\alpha$ such that $S_\gamma$ commutes with $\HGQDM$ everywhere except at the ends of $\gamma$. Therefore, for closed loops $\Gamma$, $[ S_\Gamma,\HGQDM]=0$.
...
The two unique fixed-number constraints on the square lattice $n_0 = 1$ and $n_0=2$ lead to the QDM and the quantum full-packed loop model (QFPLM), respectively. For such GQDMs with a number constraint, we write the gauge transformation as:
\begin{equation}
G_v = e^{i \alpha \left( n_v -n_0 \right)}; \notag
\end{equation}
as the states in the GQDM Hilbert space are invariant under $G_v$ for all $0 \leq \alpha \leq 2 \pi$, this represents a local $U(1)$ gauge symmetry. The Hamiltonian for these models gives dynamics to parallel dimers on the plaquettes $\bplqt$ of the lattice:
\begin{equation}
\HQDM\left(t,v\right) \equiv \HGQDM \biggl( \bigl\{ \left(\vrtplqt,\hrzplqt\right) \bigr\},\bigl\{ \left(t,v\right) \bigr\} \biggr)
\end{equation}
...
\end{equation}
thus there are $\mathcal{O} (L_x L_y)$ distinct topological sectors. Since the plaquette flip term in \Eqref{eq:HQDM} is ergodic in each of these topological sectors on the square lattice~\cite{????}, each of these topological sectors corresponds to a unique RK ground state where $\Omega$ is labeled by $(\Phi_x,\Phi_y)$.

To understand defects in these models, we define background flux $\tilde{\Phi}_\ell$
\begin{equation}
\tilde{\Phi}_\gamma \equiv -\frac{n_0}{4} \sum_{\ell \in \gamma} \rm{sgn}\left(\ell,\gamma\right) 
\end{equation}
and the total charge $Q_\Gamma$ enclosed in a closed loop $\Gamma$ as
\begin{equation}
Q_\Gamma \equiv \Phi_\Gamma+ \tilde{\Phi}_\Gamma.
\end{equation}
(Alternately can think of the source of $\Phi_\Gamma$ to be static charges of charge $+n_0$ ($-n_0$) sitting at the verticies of the $A$($B$) sub-lattice.) In this language, the gauge constraint requires each vertex to have $Q_v = 0$; thus defects which violate the constraint have $Q_v \neq 0$. A table of the charges associated with defects in the QDM and QFPLM is listed in Table \ref{tab:defects}.
...
\begin{table}
  \begin{tabular}{| l | c | c | c |}
	...
	\hline
  \end{tabular}
  \caption{Defects and their associated charges on the A sub-lattice in both the QDM and QFPLM. Defects on the B sub-lattice have opposite charge.}
  \label{tab:defects}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We now consider relaxing the local constraint to a parity constraint such that $n_v \mod 2 = n_P$ for $n_P = 0,1$; the resulting GQDM are the even \& odd parity Toric Code (or Ising gauge theory). These models have a correspondingly reduced gauge symmetry which can be written as
\begin{equation}
G_v^P = e^{\pm i \pi \left(n_v-n_P\right)}, \notag
\end{equation}
and the gauge symmetry is now reduced from $U(1)$ to $Z_2$. The set of allowed dimer resonances $\mathcal{C}_{\rm{TC}}$ is expanded accordingly \textcolor{red}{Owen: Add symbols for 4-0 resonance, and 1-3 resonances and 2-2 resonsnce}:
\begin{equation}
\mathcal{C}_{\rm{TC}} \equiv \bigl\{ \left(\vrtplqt,\hrzplqt\right),\left(\vrtplqt,\hrzplqt\right),\left(\vrtplqt,\hrzplqt\right),\left(\vrtplqt,\hrzplqt\right) \bigr\}
\end{equation}
...
\begin{equation}
\HTC\left(t\right) \equiv \HGQDM \biggl( \mathcal{C}_{\rm{TC}},\bigl\{ \left(t,0\right) \bigr\} \biggr).
\end{equation}
In this case the equal superposition RK wavefunction is the exact ground state occurs in the absence of a potential energy term.
