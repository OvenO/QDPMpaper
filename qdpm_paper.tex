...
\end{equation}
where $\hat{T}$ is an off-diagonal kinetic term that preserves the constraint, $\hat{V}$ is a diagonal potential energy term, and $v$ and $t$ parametrize the strength of each term. In the simplest case, $\hat{T}$ involves the minimum non-trivial dynamics that preserve the constraint. For example in the QDM $\hat{T}$ is the sum of all possible ``plaquette flip" terms acting on plaquettes with parallel dimers:
\begin{equation}
    \label{}
