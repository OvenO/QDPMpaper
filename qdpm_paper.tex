...
\section{The Quantum Dimer Pentamer Model}
    ...
    \subsection{QDPM Hamiltonian}
        ...
        \end{equation}
        Specificaly the penatamers pentamer terms are constructed with the dynamics shown in
...
\section{Estimating the Gap}
    An exponetial decay of the spacial correlations shown in the previous sections 
    suggeste a gap in the spectrum. However we would like to know if the lowest lying excitations are in the vison
    or the dimer sector.  The energy gap of the dimer and vison exsitations can be estimated by investigating the dimer and
    vison imaginary time correlations respectivly as outlined by Henley \cite{Henley2004a}. 
    One changlenge in the gap estimates from the imaginary time correlations is going far enough out
    in imaginary time. In appendix \ref{appendix_gap_err} we discuss our proccess in determining
    that this systematic error is sufficinetly small which is necissary when compairing gap
    estimates for different excitatoins.
    ...
    In this paper we only investigate the high symetry points in the Brillouin zone. 
